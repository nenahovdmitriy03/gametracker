...
// Р§РёС‚Р°С‚СЊ РґРѕСЃС‚РёР¶РµРЅРёСЏ РѕРґРЅРѕР№ РёРіСЂС‹ РёР· Р»РѕРєР°Р»СЊРЅРѕРіРѕ РєРµС€Р°
ipcMain.handle('steam:localAchievements', async (_e, { steamPath, steamId, appid, apiKey }) => {
  try {
    const { achievements: rawAchievements, accountId: usedId, statsDir } = readSteamLocalAchievementsForApp(steamPath, steamId, appid);
    let achievements = rawAchievements;
    if (!achievements) return { ok: false, error: `Р¤Р°Р№Р»С‹ СЃС‚Р°С‚РёСЃС‚РёРєРё РЅРµ РЅР°Р№РґРµРЅС‹ (РїР°РїРєР°: ${statsDir})` };
    achievements = await enrichLocalSteamAchievements(achievements, appid, apiKey, { translate: true });

    return {
      ok: true, achievements, accountId: usedId,
...
ipcMain.handle('steam:import', async (_e, { apiKey, steamId, steamPath }) => {
  try {
    ...
    const allGames = games.map(g => enrichedMap[g.appid] || { ...g, achievements: [], achievementsUnlocked: 0, achievementsTotal: 0 });
